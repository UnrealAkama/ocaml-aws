...
let to_http service region req =
  let uri =
    Uri.add_query_params
      ...
      (List.append [("Version", ["2014-11-01"]); ("Action", ["CreateGrant"])]
         (Util.drop_empty
            (Uri.query_of_encoded
               (Query.render (CreateGrantRequest.to_query req))))) in
  (`POST, uri, [])
let of_http body =
  try
    ...
    try
      Util.or_error (Util.option_bind resp CreateGrantResponse.parse)
        (let open Error in
           BadResponse
             {
               body;
               message = "Could not find well formed CreateGrantResponse."
             })
    with
            ...
            (BadResponse
               {
                 body;
                 message =
                   ("Error parsing CreateGrantResponse - missing field in body or children: "
                      ^ msg)
               })
  with
  | Failure msg ->
