...
let to_http service region req =
  let uri =
    Uri.add_query_params
      ...
      (List.append [("Version", ["2014-11-01"]); ("Action", ["ReEncrypt"])]
         (Util.drop_empty
            (Uri.query_of_encoded
               (Query.render (ReEncryptRequest.to_query req))))) in
  (`POST, uri, [])
let of_http body =
  try
    ...
    try
      Util.or_error (Util.option_bind resp ReEncryptResponse.parse)
        (let open Error in
           BadResponse
             {
               body;
               message = "Could not find well formed ReEncryptResponse."
             })
    with
            ...
            (BadResponse
               {
                 body;
                 message =
                   ("Error parsing ReEncryptResponse - missing field in body or children: "
                      ^ msg)
               })
  with
  | Failure msg ->
...
let parse_error code err =
  let errors =
    ...
    Errors_internal.InvalidCiphertext;
